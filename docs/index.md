...
![Insufficient resources flow](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/aau-claaudia/OpenStackGateway/main/docs/diagrams/insufficient-resources-flow.iuml)

This flow illustrates how a stack lifecycle can look. 
...
If the max lifetime for a shutoff instance is reached the whole stack is deleted.
...
If the response from ucloud is `Insufficient resources`, the stacks instance is shut off.

If this stack can successfully charge later on, the instance is restarted.

### Heartbeat
This is run every 5 minutes to add a log entry to splunk.
An alarm is configured to monitor this log entry and make sure the service is running.

### cleanUpFailedStacks

Deletes stacks that failed to create or delete.
An update is sent to ucloud when delete is successful

### deleteNotChargedStacks

Delete the stacks that are shutdown and has not been charged for a set amount of time.
The time is configured under `janitor` in the config.
`deleteShutoffInstanceAfterDays` is the default number of days a stack can have a shutoff instance before it is deleted.
`flavorLifetimes` is flavor specific lifetimes that overrules the default.

### sendCreditDeficitUpdates

When a stack has had its instance shutoff by failing to charge, this task will send a message to be shown on the ucloud interface.
It calculates how many resources are needed to restart the instance and continue charging.
